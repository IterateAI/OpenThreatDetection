        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {currentData.map((item, index) => (
            <tr key={index} className={index % 2 === 0 ? 'bg-gray-light' : 'bg-white'}>
              <Link href={routerBase+"dashboard/notifications/" + item._id}><td className="px-6 py-4 whitespace-no-wrap">{item.video_name}</td></Link>
              <td className="px-6 py-4 whitespace-no-wrap">{item.status}</td>
              <td className="px-6 py-4 whitespace-no-wrap">{item.date_time}</td>
              <Link href={routerBase+"dashboard/notifications/" + item._id}><td className="px-6 py-4 whitespace-no-wrap">{item.image_path}</td></Link>
